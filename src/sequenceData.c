...
/* everything that is not [acgtACGT] is flagged by a 0 */
int *getRestrictedDnaDictionary(int *dic){
  if(dic == NULL)
    dic = (int *)emalloc(DICSIZE * sizeof(int));
...
Sequence *getPermanentNextSequence(FILE *fp){
  ...
  if(line == NULL) {
    ...
    if(nread == -1) {
      fprintf(stderr, "ERROR[getPermanentNextSequence]: Problem reading first header line of FASTA file.\n");
      exit(-1);
    }
  }
  ...
  while((nread = getline(&line, &len, fp)) != -1) {
    if(line[0] == '>') {
      sequence->seq = (char *)erealloc(sequence->seq, seqlen + 1);
      sequence->seq[seqi] = '\0';
      sequence->len = seqi;
      return sequence;
    }
    ...
    if(line[nread-1] == '\n')
      nread--;
    seqlen += nread;
    sequence->seq = (char *)erealloc(sequence->seq, seqlen);
    for(i = 0; i < nread; i++) {
...
Sequence *readFasta(int fd){
  ...
  while((c = read(fd,buf,BUFSIZ)) > 0){
	...
	s->borders = (size_t *)erealloc(s->borders,(s->numSeq+1)*sizeof(size_t));
	if(s->numSeq >= 1){
...
Sequence *getNextSequence(FILE *fp){
  ...
  if(line == NULL) {
    nread = getline(&line, &len, fp);
    if(nread == -1) {
      fprintf(stderr, "ERROR[getNextSequence]: Problem reading first header line of FASTA file.\n");
      exit(-1);
    }
  }
  ...
  while ((nread = getline(&line, &len, fp)) != -1) {
    if(line[0] == '>'){
      sequence->len++;
      sequence->seq = (char *) erealloc(sequence->seq,sequence->len * sizeof(char));
    ...
    if(line[nread-1] == '\n')
      nread--;
    sequence->len += nread; 
    sequence->seq = (char *)erealloc(sequence->seq, sequence->len);
    for(i = 0; i < nread; i++) {
      sequence->seq[seqi++] = line[i];
    }
  }
...
void prepareSeq(Sequence *sequence){
  ...
  sequence->borders = (size_t *)erealloc(sequence->borders,2*sequence->numSeq*sizeof(size_t));
...
/* /\* randomizeSeq: in-place randomization of sequence string of query and subject sequence(s) of seq  */
/*  * deals with: */
/*  * - multiple sequences */
